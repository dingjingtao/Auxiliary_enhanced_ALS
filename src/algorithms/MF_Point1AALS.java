 ...
 * @author xiangnanhe
 */
public class MF_Point1AALS extends TopKRecommender {
	/** Model priors to set. */
  ...
  public double w_new = 1;
	...
	public MF_Point1AALS(SparseMatrix trainMatrix, ArrayList<Rating> testRatings, SparseMatrix trainSideMatrix, 
...
//				Wneg.setValue(u, i, w1);
	...
	protected void update_user(int u) {
		...
		ArrayList<Integer> itemList_side = trainSideMatrix.getRowRef(u).indexList();
		...
		for (int i : itemList_side) {
			...
			rating_items_side[i] = trainSideMatrix.getValue(u, i);
			w_items_side[i] = w1;
		}
	...
	protected void update_item(int i) {
		...
		ArrayList<Integer> userList_side = trainSideMatrix.getColRef(i).indexList();
		...
		for (int u : userList_side) {
			...
			rating_users_side[u] = trainSideMatrix.getValue(u, i);
			w_users_side[u] = w1;
		}
