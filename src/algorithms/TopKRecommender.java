...
public abstract class TopKRecommender {
	...
	public void evaluate_topk(ArrayList<Rating> testRatings) {
		int []Ks = {1,2,5,10,20,50,100};
		long start = System.currentTimeMillis();
		assert userCount == testRatings.size();
		for (int u = 0; u < userCount; u ++)
			assert u == testRatings.get(u).userId;
		
		DenseVector []thishits = new DenseVector [7];
		DenseVector []thisndcgs = new DenseVector [7];
		DenseVector []thisprecs = new DenseVector [7];
		for (int i = 0;i < 7;i++) {
			thishits[i] = new DenseVector(userCount);
			thisndcgs[i] = new DenseVector(userCount);
			thisprecs[i] = new DenseVector(userCount);
		}
		
		for (int count = 0;count < testRatings.size();count ++) {
			int u = testRatings.get(count).userId;
			double[] result = new double[3];
			int gtItem = testRatings.get(count).itemId;
			...
			int countLarger = 0;
			for (int i = 0; i < itemCount && countLarger <=100; i++) {
			...
			ArrayList<Integer> rankList = ignoreTrain ? 
					CommonUtils.TopKeysByValue(map_item_score, 100, trainMatrix.getRowRef(u).indexList()) : 
					CommonUtils.TopKeysByValue(map_item_score, 100, null);
			
			for (int i = 0;i < 7;i++) {
				List<Integer> rankListK = rankList.subList(0, Ks[i]);
				result[0] = getHitRatio(rankListK, gtItem);
				result[1] = getNDCG(rankListK, gtItem);
				result[2] = getPrecision(rankListK, gtItem);
				thishits[i].set(u, result[0]);
				thisndcgs[i].set(u, result[1]);
				thisprecs[i].set(u, result[2]);
			}
		}
		}
		...
		for (int i = 0;i < 7;i++) {
		res[0][i] = thishits[i].mean();
		res[1][i] = thisndcgs[i].mean();
		res[2][i] = thisprecs[i].mean();
		System.out.printf("Top-%d\t<hr, ndcg, prec>:\t %.4f\t %.4f\t %.4f\n",Ks[i], res[0][i], res[1][i], res[2][i]);
		}
